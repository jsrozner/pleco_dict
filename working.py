...
from util import pp as pp

kImportDatabase = "./database/shuowen1.p"
#kWriteFile = "./dict_output/shuowen_multi_withpinyin2.txt"
...
class DictWriter:
    def __init__(self, file):
        if os.path.isfile(file): exit(print("File exists already. Exit!"))
...
def main():
    ...
    for key in database:
        ...
        """"
        if "related_chars" in database[key] and len(database[key]["related_chars"]) > 1:
            print("multiple related")
            pp.pprint(database[key]["related_chars"])
            print(key)
        """



        for k in database[key]["explanations"].keys():
            countingdata.add(k)

    print("Created %d entries" % counter)
    pp.pprint(countingdata)
